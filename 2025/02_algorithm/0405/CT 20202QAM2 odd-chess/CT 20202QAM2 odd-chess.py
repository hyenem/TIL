...
* btk 매개변수로 받았던 상어 좌표가 바뀐다고 생각해서 틀렸습니다
* 백트래킹할 때 변하는 변수 유지되는 변수 확인할 것
...
def move(sx, sy):
    for i in range(1, 17):
        ...
        for _ in range(8):
            dx, dy = dxdy[d]
            nx, ny = x+dx, y+dy
            if not(0<=nx<4 and 0<=ny<4) or (sx, sy)==(nx, ny):
                d = (d+1)%8
                continue
...
def btk(x, y, d, acc):
    ...
    while 0<=nx+dx<4 and 0<=ny+dy<4:
        ...
        btk(nx, ny, tmp[nx][ny][1], acc+tmp[nx][ny][0])
...
for i in range(4):
    for j in range(4):
        arr[i][j] = (data[i][2*j], data[i][2*j+1]-1)
        loc[data[i][2*j]]=(i, j)
