...
* 인덱스 하나 잘못써서 디벅깅하는데 시간이 걸림

[ 나아진 점 ]
* 매번 새로운 배열 안만들어도 되게 했다
* 주기처리 미리 잘 헤뒀다
...
def move():
    for i in range(K):
        ...
        if arr[nx][ny]!=-1 and arr[nx][ny]<i:
            if gompang[arr[nx][ny]][4]<b:
                die[arr[nx][ny]]=1
                arr[nx][ny]=i
                gompang[i]=(nx, ny, s, d, b)
            else :
                die[i]=1
        else:
            arr[nx][ny]=i
...
gompang =[tuple(map(int, input().split())) for _ in range(K)]
arr = [[-1]*M for _ in range(N)]
for i in range(K):
    x, y, s, d, b = gompang[i]
    gompang[i]=(x-1, y-1, s, d-1, b)
    arr[x-1][y-1]=i
die = [0]*K
