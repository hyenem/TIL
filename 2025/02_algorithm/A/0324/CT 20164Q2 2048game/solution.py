...
* 구상에 시간을 거의 안쓰게 되니 시간이 많이 단축되네
구현 : 11분 / 디버깅 : 16분
* while문이 j를 바탕으로 도는데, j를 선언조차 하지 않은 이슈가 있음
    -> 이것떄문에 틀렸는데 애먼 것 고치다가 문제발생
    -> 수정하고 맞았습니다
* zip없이도 전치하는거 잘하지롱~
* del 뒤에서 부터 처리하ㄴㄴ는 것 잘해졌지롱~

[ 채크리스트 ]
[V] 하드코딩 한 부분 다시 검증하기(없습니다.)
...
def play(cnt, arr):
    ...
    if cnt == 0: delete(newarr)
    pop(newarr)
    play(cnt+1, newarr)

    newarr = [ele[::-1] for ele in arr]
    if cnt == 0: delete(newarr)
    pop(newarr)
    play(cnt + 1, newarr)
